    ...
    return (
        ...
        >
            <div className="bg-gray-400 w-full h-[60px] flex items-center justify-between pl-3 pr-5">
                <h1 className="text-color56 min-w-max text-2xl font-roboto-slab font-bold">
                    Categories
                </h1>
            ...
            <div>
                {coursesArray.sort((a, b) => a.text.localeCompare(b.text)).map(({id, text}) => (
                    <Link
                        onClick={toggleMenu}
                        className={`block px-3 py-2 rounded-md hover:text-color56 font-bold ${+categoryId === id ? "text-color56" : "text-color12"}`}
